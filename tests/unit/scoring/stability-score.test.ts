 ...
 *
 * Story: T1-S1-B-03.
 * Patent claims: CS-1 (stability ranking), CS-14 (stability component of winner gate).
 *
 * AC fixtures:
...
describe("deriveStabilityScore (scoring_v1)", () => {
  describe("AC fixtures", () => {
    it("test_claim_CS_14_stability_score_high_mass_and_high_confidence_yields_high_stability", () => {
      // eff_mass = 50, conf_lb_bp = 9000. mass*100 = 5000. min(5000, 9000) = 5000.
      const s = deriveStabilityScore(Decimal4.parse("50"), 9000);
      expect(s).toBe(5000);
    });

    it("test_claim_CS_14_stability_score_high_mass_but_low_confidence_bottlenecks_on_confidence", () => {
      // eff_mass = 100, conf_lb_bp = 1000. mass*100 = 10000. min(10000, 1000) = 1000.
      const s = deriveStabilityScore(Decimal4.parse("100"), 1000);
      expect(s).toBe(1000);
    });

    it("test_claim_CS_14_stability_score_high_confidence_but_low_mass_bottlenecks_on_mass", () => {
      // eff_mass = 5, conf_lb_bp = 9500. mass*100 = 500. min(500, 9500) = 500.
      const s = deriveStabilityScore(Decimal4.parse("5"), 9500);
      expect(s).toBe(500);
    });

    it("test_claim_CS_14_stability_score_zero_mass_yields_zero", () => {
      const s = deriveStabilityScore(Decimal4.ZERO, 9500);
      expect(s).toBe(0);
    });

    it("test_claim_CS_14_stability_score_zero_confidence_yields_zero", () => {
  ...
  describe("clamping and range validation", () => {
    ...
    it("test_claim_CS_14_stability_score_rejects_non_integer_confidence", () => {
      expect(() => deriveStabilityScore(Decimal4.parse("1"), 5000.5)).toThrow(/integer/);
    });
  });

  describe("fractional mass truncation", () => {
    it("test_claim_CS_14_stability_score_truncates_fractional_mass_toward_zero", () => {
    ...
    it("test_claim_CS_14_stability_score_tiny_mass_is_still_representable", () => {
  ...
  describe("version pinning", () => {
    it("test_claim_CS_14_stability_score_accepts_current_scoring_version_explicitly", () => {
      const s = deriveStabilityScore(Decimal4.parse("10"), 5000, CURRENT_SCORING_VERSION);
      expect(s).toBe(1000);
    });

    it("test_claim_CS_14_stability_score_rejects_unknown_scoring_version", () => {
      expect(() => deriveStabilityScore(Decimal4.parse("10"), 5000, "scoring_v99"))
        .toThrow(/unknown scoring_version/);
    });

    it("test_claim_CS_14_stability_score_is_deterministic_across_repeated_calls", () => {
